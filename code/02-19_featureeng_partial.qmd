...
fe_month <- fieldweather %>%
  ...
  mutate(date = as.Date(date_chr, format = "%Y/%j")) %>%
...
fe_month_sum <- fe_month %>%
  group_by(year, site, strength_gtex, month_abb) %>%
  # Mean for rate/intensity variables
...
fe_month_sum_wide <- fe_month_sum %>%
  pivot_wider(names_from = month_abb,
              values_from = c(dayl.s, srad.wm2, tmax.c, tmin.c, vp.pa, prcp.mm)) %>%
